...
tape( 'main export is a function', function test( t ) {
	...
	t.equal( typeof iterLog1p, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function returns an iterator protocol-compliant object (value+done)', function test( t ) {
	...
	it = iterLog1p( createIterator( values ) );
	t.equal( it.next.length, 0, 'has zero arity' );
...
tape( 'the function returns an iterator protocol-compliant object which evaluates the natural logarithm of `1+x` for each iterated value', function test( t ) {
	var expected;
	var it;
	var N;
	var x;
	...
	N = 101;
	it = iterLog1p( linspace( 0.0, 1000.0, N ) );
	t.equal( it.next.length, 0, 'has zero arity' );

	x = linspace( 0.0, 1000.0, N );
	for ( i = 0; i < N; i++ ) {
		r = it.next();
		expected = log1p( x.next().value );
...
tape( 'the function returns an iterator protocol-compliant object which evaluates the natural logarithm of 1+x for each iterated value (value+done)', function test( t ) {
	var expected;
	var actual;
	var values;
	var it;
	...
	expected = [
		{
			'value': log1p( values[ 0 ] ),
			'done': false
		},
		{
			'value': log1p( values[ 1 ] ),
			'done': false
		},
		{
			'value': log1p( values[ 2 ] ),
			'done': false
		},
		{
			'value': log1p( values[ 3 ] ),
			'done': true
		}
	];
